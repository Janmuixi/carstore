<template>
    <div
        class="flex min-h-screen items-start sm:items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 p-6"
    >
        <UCard
            class="w-full max-w-6xl p-4 sm:p-6 shadow-2xl rounded-2xl bg-white relative"
        >
            <!-- Back and Actions -->
            <div class="flex items-center justify-between mb-4">
                ...
                <UButton
                    ...
                    @click="router.push('/admin/new-car')"
                >
                    Add new car
                </UButton>
            </div>
            ...
            <!-- Title -->
            <h1 class="text-3xl sm:text-4xl font-extrabold mb-4 text-gray-800">
                Cars
            </h1>
            ...
            >
                <UCard
                    ...
                    :key="car.id"
                    class="p-4 rounded-xl shadow hover:shadow-lg transition-shadow cursor-pointer"
                    @click="router.push(`/admin/car-details/${car.id}`)"
                >
                    <!-- Cover Image -->
                    <div
                        class="w-full h-40 sm:h-44 rounded-lg overflow-hidden bg-gray-100 mb-3 flex items-center justify-center"
                    >
                        <img
                            v-if="coverSrcMap[car.id]"
                            :src="coverSrcMap[car.id]"
                            alt="Car image"
                    ...
                    <div
                        class="text-sm text-gray-600 line-clamp-3 mb-3"
                    ...
                    <div
                        class="grid grid-cols-2 gap-2 text-sm text-gray-700 mb-3"
                    >
                        <div>
                        ...
                        <div>
                            <span class="font-semibold">Transm.:</span>
                    ...
                    <div class="mt-3 pt-3 border-t border-gray-200">
                        <UButton
                            ...
                            class="w-full"
                            @click.stop="
                                router.push(`/admin/car-details/${car.id}`)
                            "
                        >
                            View Details
                        </UButton>
...
const filteredCars = computed(() => {
    ...
    return cars.value.filter((c) => {
        const haystack =
            `${c.make} ${c.model} ${c.year} ${c.description} ${c.fuel} ${c.transmission} ${c.color}`.toLowerCase();
